...
**更新日期**: 2026-01-28  
**范围**: Epic 3 Story 3.1–3.5（录入、列表、编辑、删除、搜索筛选）、Epic 2 会话与访问控制
...
1. 在账目列表上方有 **搜索框**（提示：搜索（备注/金额/类别））和 **类别下拉**（全部 / 餐饮 / 交通 / 购物 / 其他）、**「清除筛选」** 按钮。
2. **按关键词**：在搜索框输入备注中的文字、金额数字或类别名，列表应实时只显示匹配的记录。
3. **按类别**：在筛选类别下拉选择「餐饮」，列表应只显示类别为餐饮的账目；选「全部」则显示全部。
4. **组合**：同时选择类别「交通」并输入关键词（如某金额），列表显示同时满足条件的记录。
5. 有筛选时，下方出现 **「当前筛选: 类别=xxx, 关键词=xxx」** 提示。
6. 点击 **「清除筛选」**：搜索框清空、类别恢复「全部」、提示消失、列表显示全部账目。
...
- 搜索框与类别筛选实时生效；清除筛选后列表恢复全部；有筛选时显示当前筛选提示。
